...
export function _embind_register_constant(this: InstantiatedWasm, namePtr: number, typePtr: number, valueAsWireType: WireTypes): void {


    _embind_register(this, namePtr, async (constName) => {
        // Wait until we know how to parse the type this constant references.
        const [type] = await getTypeInfo<[EmboundRegisteredType]>(typePtr);
